...
st.set_page_config(page_title="Stützen-Stütze", layout="centered")
...
with st.container():
    ...
    st.write("Jegliche Berechnungen dienen der Vordimensionierung und sind keine vollständige Überprüfung der Statik ihres Entwurfs.")
...
with st.container():
    ...
    traegerlaenge = st.number_input("Gib die Länge der Einfeldträger ein:")
    EF = st.selectbox("Wähle den Eulerfall der Stütze aus", ["1","2","3","4"])
    FL = st.number_input("Gebe die auf die Dachfläche wirkende Flächenlast in kN/m^2 ein:")

with st.container():
    lef = int(stuetzenabstand) * int(traegerlaenge)/2 
    st.write("Die Größe des Lasteinzugsfeldes sind " + str(lef) + " Quadratmeter.")
